...
The voting module is per-track; we also need to prune tracks not seen for 90 frames (DESIGN.md §5.4) and decide *when* to classify.
...
**Step 1:** Use `npm create vite@latest frontend -- --template react-ts` as the *reference*, but write the files by hand so versions are pinned (`react@18.3.x`, `vite@5.x`, `tailwindcss@3.4.x`, `typescript@5.5.x`). Add `vitest@2.x` for tests.
